...
TEST_SUITE(MultiThreadTimers) {
  ...
  SIMPLE_TEST(DoNotBurnCpu) {
    ...
    course::test::ProcessCPUTimer cpu;
  ...
  SIMPLE_TEST(PriorityQueue) {
    ...
    static constexpr size_t kTimers = 5;
    std::array<size_t, kTimers> delays = {5, 2, 3, 1, 4};
    ...
    for (auto t : delays) {
      runtime::SetTimer(mt, 1s * t, [&wg, &c, t] {
        ASSERT_EQ(c, t - 1);
        c = t;
        wg.Done();
      });
